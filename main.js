...
class QueueObject {
    ...
    async callback(_callback) {
        try {
            ...
            const parent_uri = this.url.replace(/([^\/]*\?.*$)|([^\/]*$)/g, '');
            ...
            if (/^http.*/.test(segment.uri)) {
                ...
                if (mes && mes.length >= 1) {
                    ...
                    uri_ts = parent_uri + (parent_uri.endsWith('/') || segment.uri.startWith('/') ? '' : "/") + segment.uri;
                }
            ...
            const fileName = `${((this.idx + 1) + '').padStart(6, '0')}.ts`;//生成ts片段文件的文件名
            const filePath = path.join(this.dir, fileName);
            const filePath_dl = path.join(this.dir, fileName + ".dl");//下载的时候使用.dl后缀的文件名，下载完成后重命名

            logger.debug(`2 ${segment.uri}`, `${fileName}`);

            //按照三次retry设定,文件下载失败会继续重新尝试
            for (let index = 0; index < 3 && !fs.existsSync(filePath); index++) {
                if (/^file:\/\/\//.test(uri_ts)) {//本地已经完成下载(符合暂停后又继续下载的情况)
                    fs.copyFileSync(uri_ts.replace(/^file:\/\/\//, ''), filePath_dl);
                } else {//本地对应ts文件没有下载(常规的初次下载情况)
                    ////使用aria2Client下载ts文件
                    // const _headers = [];
                    // if (this.headers) {
                    //     for (const _key in this.headers) {
                    ...
                    //break;
                    await download(uri_ts, this.dir, {filename: fileName + ".dl", timeout: httpTimeout, headers: this.headers, agent: proxy_agent}).catch((err) => {
                        logger.error(err)
                        if (fs.existsSync(filePath_dl)) fs.unlinkSync(filePath_dl);
                    });
                }
                if (!fs.existsSync(filePath_dl)) continue;
                if (fs.statSync(filePath_dl).size <= 0) {//下载得到的文件为空
                ...
                if (segment.key != null && segment.key.method != null) {
                    ...
                    if (!this.myKeyIV && !fs.existsSync(aes_path)) {
                        ...
                        if (/^http/.test(this.url) && !/^http.*/.test(key_uri) && !/^\/.*/.test(key_uri)) {
                            ...
                            let fileDir = this.url.replace('file:///', '').replace(/[^\\/]+$/, '');
                            let key_uri_ = path.join(fileDir, key_uri);
                            if (!fs.existsSync(key_uri_)) {
                                const me = key_uri.match(/([^\\\/\?]+)(\?|$)/i);
                                if (me && me.length > 1) {
                        ...
                        if (/^http/.test(key_uri)) {
                            await download(key_uri, this.dir, {filename: "aes.key", headers: this.headers, timeout: httpTimeout, agent: proxy_agent}).catch(console.error);
            ...
            if (fs.existsSync(filePath)) {//下载成功,执行回调函数
                this.then && this.then();
            } else {
                this.catch && this.catch();
            }
...
async function startDownload(object, iidx) {
    ...
    if (!taskName) {//如果没有设置文件名,则根据当前时间戳自动生成
        taskName = `${id}`;
    }
    ...
    //并发 5 个线程下载
    const tsQueues = async.queue(queue_callback, 5);

    const count_seg = parser.manifest.segments.length;//片段总数
    ...
    for (let iSeg = 0; iSeg < segments.length; iSeg++) {
        ...
        qo.then = function () {
            ...
            if (video.success) {
                mainWindow.webContents.send('task-notify-update', video);//视频下载成功
            }
        };
        qo.catch = function () {
            if (this.retry < 5) {
                ...
                mainWindow.webContents.send('task-notify-end', video);//视频下载失败
